type OnMenuShareAppMessageParams = WxFnCommonParams &
  ShareContent & {
    success?: () => void; // 用户确认分享后执行的回调函数
    cancel?: () => void; // 用户取消分享后执行的回调函数
  };

type OnMenuShareWechat = WxFnCommonParams &
  ShareContent & {
    success?: () => void; // 用户确认分享后执行的回调函数
    cancel?: () => void; // 用户取消分享后执行的回调函数
  };

type onMenuShareTimeline = WxFnCommonParams &
  Omit<ShareContent, "desc"> & {
    success?: () => void; // 用户确认分享后执行的回调函数
    cancel?: () => void; // 用户取消分享后执行的回调函数
  };
