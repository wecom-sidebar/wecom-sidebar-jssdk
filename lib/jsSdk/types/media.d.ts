type GetLocalImgDataParams = WxFnCommonParams & {
...
type ChooseImageParams = WxFnCommonParams & {
  count: number; // 默认9
...
type UploadImageParams = WxFnCommonParams & {
...
type DownloadImageParams = WxFnCommonParams & {
...
type StopRecordParams = WxFnCommonParams & {
  success?: WxFnCallback<{ localId: string }>;
};

type OnVoiceRecordEndParams = WxFnCommonParams & {
...
type PauseVoiceParams = {
  localId: ""; // 需要暂停的音频的本地ID，由stopRecord接口获得
};

type StopVoiceParams = {
  localId: string; // 需要停止的音频的本地ID，由stopRecord接口获得
};

type OnVoicePlayEndParams = WxFnCommonParams & {
  success?: WxFnCallback<{
    localId: string; // 返回音频的本地ID
  }>;
};

type UploadVoiceParams = WxFnCommonParams & {
...
type DownloadVoiceParams = WxFnCommonParams & {
...
type TranslateVoiceParams = WxFnCommonParams & {
