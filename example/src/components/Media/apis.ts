...
export const getLocalImgData = async (localId: string) => {
  try {
    const res = await jsSdk.asyncCall('getLocalImgData', {
      localId,
    });
...
export const chooseImage = async () => {
  try {
    const res = await jsSdk.asyncCall('chooseImage', {});
...
export const previewImage = async () => {
  try {
    jsSdk.call('previewImage', {
      current: 'https://www.baidu.com/img/PCtm_d9c8750bed0b3c7d089fa7d55720d6cf.png', // 当前显示图片的http链接
      urls: [
...
export const uploadImage = async (localId: string) => {
  try {
    const res = await jsSdk.asyncCall('uploadImage', {
...
export const downloadImage = async (serverId: string) => {
  try {
    const res = await jsSdk.asyncCall('downloadImage', {
...
export const startRecord = async () => {
  return jsSdk.call('startRecord', {});
}

export const stopRecord = async () => {
  try {
    const res = await jsSdk.asyncCall('stopRecord', {});
...
export const playVoice = (localId: string) => {
  jsSdk.call('playVoice', {localId});
}

export const pauseVoice = (localId: string) => {
  jsSdk.call('pauseVoice', {localId});
}

export const stopVoice = () => {
  jsSdk.call('stopVoice', {});
}
...
export const uploadVoice = async (localId: string) => {
  try {
    const res = await jsSdk.asyncCall('uploadVoice', {
      localId,
      isShowProgressTips: 1, // 默认为1，显示进度提示
    });
...
export const downloadVoice = async (serverId: string) => {
  try {
    const res = await jsSdk.asyncCall('downloadVoice', {
      serverId,
      isShowProgressTips: 1, // 默认为1，显示进度提示
    });
...
export const translateVoice = async (localId: string) => {
  try {
    const res = await jsSdk.asyncCall('translateVoice', {
      localId,
      isShowProgressTips: 1, // 默认为1，显示进度提示
    });
